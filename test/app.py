...
def is_booking_time_valid(date, start_time, end_time):
    ...
    # End time must be between 9:15 and 18:00
    if end_hour < 9 or (end_hour == 9 and end_minute < 15) or end_hour > 18:
        return False, 'outside_working_hours'
...
def get_room_status(room_id):
    ...
    print(f"DEBUG: Checking room {room_id} status at {current_time.strftime('%H:%M:%S')} on {current_date}")
    ...
    for booking in bookings:
        if (booking['room_id'] == room_id and 
            ...
            try:
                ...
                print(f"DEBUG: Found booking {booking_start.strftime('%H:%M')} - {booking_end.strftime('%H:%M')} by {booking.get('user_name', 'Unknown')}")
                
            except ValueError as e:
                print(f"DEBUG: Invalid time format in booking: {booking} - Error: {e}")
                continue
    
    # Sort bookings by start time
    active_bookings.sort(key=lambda x: x['start'])
    ...
    for booking in active_bookings:
        ...
        # Current time >= start AND current time < end
        if booking_start <= current_time < booking_end:
            print(f"DEBUG: Room is OCCUPIED - Current time {current_time.strftime('%H:%M:%S')} is within booking {booking_start.strftime('%H:%M')}-{booking_end.strftime('%H:%M')} by {booking['user']}")
            return 'occupied'
    
    # If we have bookings today but none are active now
    if active_bookings:
        next_booking = None
        for booking in active_bookings:
        ...
        if next_booking:
            print(f"DEBUG: Room is AVAILABLE - Next booking at {next_booking['start'].strftime('%H:%M')} by {next_booking['user']}")
        else:
            print(f"DEBUG: Room is AVAILABLE - All bookings for today have ended")
    else:
        print(f"DEBUG: Room is AVAILABLE - No bookings for today")
